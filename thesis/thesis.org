...
* Introduction
  ...
  makes it easier for multiple users to form a document together. Furthermore,
  it is considered /real-time/ if multiple users can edit the document
  simultaneously cite:ellis1989concurrency, and that changes are propagated to
  the other users within a reasonably short amount of time.
  ...
  Buffer. It is designed with developers in mind. What most developers have in
  common is that they spend a lot of time manipulating /plain text/, yet they
  use a [[http://stackoverflow.com/research/developer-survey-2016][lot of different tools]] to do so. We aim to enable real-time
  collaboration in /existing/ text editors, as opposed to developing an editor
  with real-time collaborative features. Our prototype implementation is for
  the text editor Emacs. The name Shared Buffer reflects a choice in design;
  Emacs stores text in a /buffer/; when a file is opened, its contents is put
  inside a buffer, which the user can manipulate. You may also have buffers
  that are not associated with any file. In Shared Buffer, there are is no
  notion of a file, meaning there is no centrally stored copy of the document.

  We want to develop a protocol which ideally should be portable to any text
  editor, or any program that embeds a text editor. A client-server model is
  chosen, as opposed to a fully decentralized solution. We will consequently
  move complexity to the server, if this can simplify the client-side
  algorithm. This is chosen in order to ensure portability. Furthermore,
  proving correctness for a fully decentralized solution has proven to be very
...
** Introducing the Problem
   ...
   European friend $u_0$ has an empty buffer which she precedes to insert an
   "=a=" into. Meanwhile $u_1$, the American, inserts a "=b=" in front of the
   "=a=" that just popped up in her buffer. The "=b=" eventually reaches $u_0$,
   and the end result of the interaction is that they both will be looking at a
   buffer containing "=ba=". In this scenario the buffers ended up identical,
   so we say that we reached a consistent state.

   Scenarios like the one we saw would be gracefully handled by even a naive
   approach. We can see that "=a=" was inserted prior to the "=b=" at both
   $u_0$ and $u_1$, hence they cannot have been applied concurrently. We will
   now demonstrate that the approach does not work when we introduce concurrent
   edits.
   ...
   In later chapters we will come to realize that this is not at all trivial,
   considering the highly concurrent and distributed nature of the problem. In
   order to handle this level of complexity we will rely heavily on the use of
   formal methods.

** Approach

   Most non-trivial programs contain bugs. There are many classes of bugs,
   which range in both seriousness and subtleness. If a bug is discovered in a
   running system, it implies that time has been spent actually developing that
   bug; now more time needs to be spent fixing the bug. Some bugs can even be
   integrated in the system in such a way than correcting it requires large
   parts of the system to be rewritten. Our goal is to avoid developing serious
   bugs entirely, which of course is easier said than done.

   To quote Rich Hickey's talk [[http://www.infoq.com/presentations/Simple-Made-Easy][Simple Made Easy]]:

   #+BEGIN_QUOTE
   What's true of every bug found in the field? It passed the type checker.
   What else did it do? It passed all the tests.
   #+END_QUOTE

   This is not to undermine the merit these techniques, but rather to show that
   there are limits to what they can do. The bugs that are caught by tests do
   so because someone thought it was a good idea to write the test that caught
   it. In other words, no matter what bug, no one thought to test the scenario
   that would have shed light on it. We do not want to rely on ourselves to
   think about every relevant scenario when developing our protocol, rather we
   want to use tools that lends itself to that purpose.

   We will use a formal verification technique called model checking. This is a
   technique that requires us to write a mathematical representation of the
   system, which will be our model. We can perform exploratory searches of the
   model, and check whether certain properties holds true. A model is generally
   an abstraction of a given system, where one carefully chooses what parts of
   the system is necessary to represent, in order to prove the properties that
   are of interest.
   ...
   and distributed systems [[cite:DBLP:conf/maude/2007]]. It allows us to represent
   states in a system, and transitions between states. We can think of model
   ...
   whether or not the property holds true in every state. Further, we want to
   use Linear Temporal Logic (LTL) to express properties over paths, which are
   sequences of states.
