...
** Scenarios Described in Terms of Operations and Their Application
   ...
   $\epsilon$) and the order they operation they apply is $O_1 \circ O_0$.
   Because they perform the same operation to the same initial buffer, they
   must necessarily end up in a consistent state (i.e. end up with the same
   buffer).

   The scenario from Figure [[ref:fig:conflict0]] (page pageref:fig:conflict0) is
   identical to the previous scenario, except that the orders of operations
   have changed.
   #+BEGIN_EXPORT latex
   \begin{multicols}{2}
     From the perspective of $u_0$:
   ...
   that $u_0$ and $u_1$ end up in a inconsistent state (i.e. end up with
   different buffers).
   #+BEGIN_EXPORT latex
   \begin{multicols}{2}
...
** Algebraic Properties
   ...
   naïve algorithm (Section [[A Naïve Algorithm]]); in other words, an naïve
   algorithm would guarantee eventual consistency if the order of which
   operations are applied does not effect the end result.
