...
*** Undo
    ...
    operations that are generated locally. This problem needs to be handled on
    the client side, as undo is a editor-specific feature. The Shared Buffer
    system does not provide any special mechanisms for undo, but specific
    client implementations may provide solutions to this problem; the only
    requirement is that all changes (including undos) are reported to the
    server.
    ...
    that were generated remotely.

    We consider the problem outside the scope of this thesis, but encourage the
    idea of exploring this further in future work.
