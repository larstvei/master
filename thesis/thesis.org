...
*** Invertibility
    ...
    \begin{definition}[Inverse of an Operation]
      ...
      \[ del(i,\ c)^{-1} = ins(i,\ c) \]
      For a composed operation $O_i \circ O_j \in \mathcal{O}$, the order of the
      operations is reversed, and the operations are inverted:
      \[ (O_i \circ O_j)^{-1} = O_j^{-1} \circ O_i^{-1} \]\hfill$\dashv$
    \end{definition}
    #+END_EXPORT
...
** Transform the History
   ...
   \begin{align*}
     ...
     \\
     et(O_i, O_j \circ O_k) &= et(et(O_i, O_k), O_j) \text{\quad\ \ if $O_j$ is not composed}
     \\
...
** Summary
   ...
   The rule in Figure [[ref:fig:maude-rule]] concisely summarizes this chapter,
   as it describes the /reaction/ to the reception of a message. The reception
   of a message is modeled as the server nondeterministically picking a message
   off of the end of the message outgoing queue of a client. It adds a response
   message to the incoming queue to the client. The expression =(send O' to
   US)= similarly adds a message containing the operation =O'= on all the other
   clients incoming queues with the correct sequence number.
   ...
   number scheme (Section [[Sequence Number Scheme]]).

   Note that the rule is expressed as a /conditional/ rule =crl= in order to
   achieve the effects of a /let/-expression, commonly found in functional
